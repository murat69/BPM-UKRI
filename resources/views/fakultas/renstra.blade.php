                ...
                <div class="card">
                    ...
                    <div class="col-md-8" style="margin-top: 10px; margin-bottom: 10px;">
                        <button type="button" class="btn btn-primary buttonedit" data-toggle="modal" data-target="#myModal"
                            data-url="{{ route('file_fakultas.store') }}">
                            Tambah Data {{ $title }}
                        </button>
                    </div>
                    <!-- Modal -->
                    <div class="modal fade" id="myModal" tabindex="-1" aria-labelledby="myModal" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('file_fakultas.store') }}" method="post" class="formkirim"
                                        ...
                                        <div class="container">
                                            <div class="row justify-content-center">
                                                <div class="col-md-12">
                                                    ...
                                                    <input type="hidden" name="kategori" value="renstra" id="">
                        ...
                        <tbody>
                            ...
                            @foreach ($file as $data)
                                <tr>
                                    ...
                                    <td> {{ $data->nama }}</td>
                                    <td><a href="{{ asset('storage/upload/' . $data->file) }}"
                                    ...
                                    @endif
                                    <td>
                                        <form action="{{ route('file_fakultas.destroy', $data->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                                            <a href="" class="btn btn-sm btn-outline-success buttonedit"
                                                data-toggle="modal" data-target="#myModal"
                                                data-nama="{{ $data->nama }}" data-fakultas="{{ $data->fakultas_id }}"
                                                data-old-file="{{ $data->file }}"
                                                data-deskripsi="{{ $data->deskripsi }}"
                                                data-url="{{ route('file_fakultas.update', $data->id) }}">
                                                Edit
                                            </a> |
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.buttonajax', function() {
                ...
                var newURL =
                    "{{ route('file_fakultas', ['kategori' => 'renstra', 'kat_fak' => ':kat_fak']) }}";
            ...
            $('.status').on('change', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('file_fakultas.status') }}',
                    data: {
